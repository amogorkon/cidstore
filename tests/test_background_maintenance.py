...
async def test_background_maintenance_initialization():
    ...
    stats = store.maintenance_thread.get_stats()
    ...
    assert not store.maintenance_thread.get_stats()["running"]
...
async def test_background_sort_unsorted_regions():
    ...
    store = make_store()
    store.maintenance_thread.stop()  # Stop automatic background
    ...
    test_maintenance = BackgroundMaintenance(
        store, interval=1, sort_threshold=3, merge_threshold=2
    )
    ...
    try:
        ...
        if hasattr(store, "_find_bucket_id"):
            ...
            unsorted_count_after = store.get_unsorted_count(bucket_id)
...
async def test_background_maintenance_integration():
    ...
    assert hasattr(store, "gc_thread")
    assert hasattr(store, "maintenance_thread")
    assert store.gc_thread.is_alive()
    assert store.maintenance_thread.is_alive()
