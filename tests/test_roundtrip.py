...
async def test_full_roundtrip(tmp_path):
    path = tmp_path / "roundtrip.h5"
    # wal_path intentionally unused in this test harness
    storage = Storage(str(path))
    wal = WAL(None)
    store = CIDStore(storage, wal=wal)
    # Insert a variety of keys/values
    for i in range(100):
        await store.insert(E.from_str(f"key{i}"), E.from_int(i))
    # Delete some
    for i in range(0, 100, 3):
        await store.delete(E.from_str(f"key{i}"))
    # Close and reopen
    del store
    ...
    for i in range(100):
        ...
        got = await store2.get(E.from_str(f"key{i}"))
        assert list(got) == expected


async def test_persistence_and_recovery(tmp_path):
    path = tmp_path / "persist.h5"
    # wal_path intentionally unused in this test harness
    storage = Storage(str(path))
    wal = WAL(None)
    store = CIDStore(storage, wal=wal)
    await store.insert(E.from_str("persist"), E.from_int(123))
    del store
    storage2 = Storage(str(path))
    wal2 = WAL(None)
    store2 = CIDStore(storage2, wal=wal2)
    got = await store2.get(E.from_str("persist"))
    assert E.from_int(123) in list(got)
