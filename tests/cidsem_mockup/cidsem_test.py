...
def main():
    ...
    for i in range(0, len(triples), BATCH_SIZE):
        ...
        if (i + BATCH_SIZE) % (BATCH_SIZE * 10) == 0:
            ...
            print(f"  Inserted {inserted:,}/{NUM_TRIPLES:,} triples "
                  f"({rate:.0f} triples/sec, {failed} failed)")
    
    insertion_time = time.time() - start_time
    print(f"✓ Inserted {inserted:,} triples in {insertion_time:.2f}s "
    ...
    print(f"Insertion time:     {insertion_time:.2f}s ({inserted / insertion_time:.0f} triples/sec)")
    print(f"Validation time:    {validation_time:.2f}s")
    print(f"Total time:         {generation_time + insertion_time + validation_time:.2f}s")
    print()
    ...
    if success_rate >= 0.99 and verification_rate >= 0.95:
        ...
        print(f"  {verification_rate * 100:.1f}% verification success (expected >= 95%)")
        sys.exit(1)
