...
async def test_hdf5_sharding_and_layout(tmp_path):
    ...
    for i in range(10000):
        await store.insert(E.from_str(f"shardkey{i}"), E.from_int(i))
...
async def test_hdf5_metrics_and_logging(tmp_path):
    from cidstore.keys import E
    from cidstore.store import WAL
    ...
    store = CIDStore(Storage(str(path)), wal=WAL(None))
    await store.insert(E.from_str("metrics"), E.from_int(1))
    # If API exposes metrics/logging, check them
    # Metrics/logging not required in new model; skip if not present


async def test_hdf5_sharded_directory_migration(tmp_path):
