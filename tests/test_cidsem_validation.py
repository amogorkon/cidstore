...
from cidstore.predicates import PREDICATE_LIMIT, PredicateRegistry
...
def test_validate_cidsem_valid_entity():
    ...
    for term in valid_terms:
        assert registry._validate_cidsem_format(term), (
            f"Should accept valid term: {term}"
        )
...
def test_register_cidsem_counter_valid():
    ...
    ds = registry.register_cidsem_counter(
        "R:usr:ownedQuantity", "Enables O(1) quantity aggregation"
    )
...
def test_register_cidsem_multivalue_valid():
    ...
    ds = registry.register_cidsem_multivalue(
        "R:usr:friendsWith", "Bidirectional graph queries"
    )
